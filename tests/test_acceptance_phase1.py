"""Acceptance-criteria tests for Phase 1 deliverables.

Each test maps 1:1 to an acceptance criterion in
``patch-tuesday-mcp-enhancements-PRD.md`` Section 4, Epics 2 and 7. Test names
encode the epic/criterion so failures point straight at the PRD.
...
def test_epic2_ac1_full_vector_parses_to_components():
    """AC: vector CVSS:3.1/AV:N/AC:L/PR:N/UI:N/S:U/C:H/I:H/A:H → AV=N, PR=N, UI=N."""
...
async def test_epic2_ac1_detail_output_exposes_components():
    """AC: parsed components are exposed in msrc_search CVE detail output."""
...
async def test_epic2_ac2_vector_filters_return_only_matches():
    """AC: attack_vector=N + privileges_required=N returns only matching CVEs."""
...
def test_epic2_ac3_malformed_vector_preserves_raw_no_exception():
    """AC: malformed vector keeps raw cvss_vector available; no exception escapes."""
...
async def test_epic2_ac4_no_vector_filters_is_backward_compatible():
    """AC: without vector filters, response shape is unchanged (additive only)."""
...
async def test_epic2_ac3_live_month_search_tolerates_mixed_vectors():
    """AC: malformed/missing vectors don't fail a monthly search."""
...
async def test_epic7_ac1_detail_includes_msrc_and_nvd():
    """AC: CVE detail references include MSRC and NVD links."""
    detail = (await msrc_search(cve="CVE-2026-41108"))["vulnerabilities"][0]
...
def test_epic7_ac2_kev_listed_cve_includes_kev_reference():
    """AC: KEV-listed CVE detail references include a CISA KEV catalog link."""
...
async def test_epic7_ac3_summary_omits_references_by_default():
    """AC: broad summary output does not carry references (context stays small)."""
    result = await msrc_search()
    assert all("references" not in v for v in result["vulnerabilities"])


def test_epic7_ac_no_extra_api_calls_for_references():
    """AC: references are generated deterministically, without network calls."""
